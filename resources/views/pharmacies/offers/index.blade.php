...
@section('title', app()->getLocale() === 'ar' ? 'عروض الصيدلية' : 'Pharmacy Offers')
@section('page-title', app()->getLocale() === 'ar' ? 'عروضي' : 'My Offers')

@section('content')
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="p-6 border-b">
        <h3 class="text-lg font-semibold text-slate-900">{{ app()->getLocale() === 'ar' ? 'العروض المرسلة' : 'Sent Offers' }}</h3>
    </div>
    <div class="p-0">
        @if($offers->count())
            <table class="min-w-full divide-y divide-gray-200">
                ...
                @foreach($offers as $offer)
                    <tr>
                        ...
                        <td class="px-6 py-3">
							<span class="px-2 py-1 text-xs rounded-full bg-slate-100 text-slate-800">{{ ucfirst($offer->status) }}</span>
						</td>
                    </tr>
                @endforeach
                </tbody>
