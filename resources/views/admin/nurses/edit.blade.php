...
<form action="{{ route('admin.nurses.update', $nurse) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl mx-auto space-y-6">
	...
	<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			...
			<div>
				<label class="block text-sm font-medium text-gray-700 mb-1">{{ app()->getLocale() === 'ar' ? 'كلمة مرور جديدة (اختياري)' : 'New password (optional)' }}</label>
				<input type="password" name="password" class="w-full border border-gray-300 rounded-lg p-2" autocomplete="new-password">
			...
			<div>
				...
				<input type="password" name="password_confirmation" class="w-full border border-gray-300 rounded-lg p-2" autocomplete="new-password">
			</div>
			<div>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div>
				...
				<select name="gender" class="w-full border border-gray-300 rounded-lg p-2">
					...
					<option value="male" {{ old('gender', $nurse->gender)=='male' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'ذكر' : 'Male' }}</option>
			...
			<div class="md:col-span-2">
				...
				<input name="address" value="{{ old('address', $nurse->address) }}" class="w-full border border-gray-300 rounded-lg p-2">
			</div>
			<div class="md:col-span-2">
			...
			<div>
				...
				<select name="qualification" class="w-full border border-gray-300 rounded-lg p-2">
					<option value="">{{ app()->getLocale() === 'ar' ? 'اختيار' : 'Select' }}</option>
					<option value="bachelor" {{ old('qualification', $nurse->qualification)=='bachelor' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'بكالوريوس' : 'Bachelor' }}</option>
					<option value="diploma" {{ old('qualification', $nurse->qualification)=='diploma' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'دبلوم' : 'Diploma' }}</option>
					<option value="technical_institute" {{ old('qualification', $nurse->qualification)=='technical_institute' ? 'selected' : '' }}>{{ app()->getLocale() === 'ar' ? 'معهد فني' : 'Technical Institute' }}</option>
				</select>
			</div>
			<div>
				<label class="block text-sm font-medium text-gray-700 mb-1">{{ app()->getLocale() === 'ar' ? 'جهة التعليم' : 'Education Place' }}</label>
				<input name="education_place" value="{{ old('education_place', $nurse->education_place) }}" class="w-full border border-gray-300 rounded-lg p-2">
			</div>
			<div>
				<label class="block text-sm font-medium text-gray-700 mb-1">{{ app()->getLocale() === 'ar' ? 'سنة التخرج' : 'Graduation Year' }}</label>
				<input type="number" name="graduation_year" value="{{ old('graduation_year', $nurse->graduation_year) }}" class="w-full border border-gray-300 rounded-lg p-2" min="1950" max="{{ date('Y')+1 }}">
			</div>
			<div>
				<label class="block text-sm font-medium text-gray-700 mb-1">{{ app()->getLocale() === 'ar' ? 'سنوات الخبرة' : 'Years of Experience' }}</label>
				<input type="number" name="years_of_experience" value="{{ old('years_of_experience', $nurse->years_of_experience) }}" class="w-full border border-gray-300 rounded-lg p-2" min="0" max="70">
			</div>
			<div class="md:col-span-2">
				<label class="block text-sm font-medium text-gray-700 mb-1">{{ app()->getLocale() === 'ar' ? 'جهة العمل الحالية' : 'Current Workplace' }}</label>
				<input name="current_workplace" value="{{ old('current_workplace', $nurse->current_workplace) }}" class="w-full border border-gray-300 rounded-lg p-2">
			</div>
			<div class="md:col-span-2">
				...
				<textarea name="certifications" rows="2" class="w-full border border-gray-300 rounded-lg p-2">{{ old('certifications', is_array($nurse->certifications) ? implode(', ', $nurse->certifications) : $nurse->certifications) }}</textarea>
			</div>
			<div class="md:col-span-2">
				...
				<textarea name="skills" rows="2" class="w-full border border-gray-300 rounded-lg p-2">{{ old('skills', is_array($nurse->skills) ? implode(', ', $nurse->skills) : $nurse->skills) }}</textarea>
			</div>
		</div>
	</div>
...
<script>
	$(function () {
		...
		@php
			...
			$selectedAreaIds = old('area_ids', $nurse->area_ids ?? []);
