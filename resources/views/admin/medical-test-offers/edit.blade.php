...
@section('title', app()->getLocale() === 'ar' ? 'تعديل عرض فحص' : 'Edit Test Offer')
@section('page-title', app()->getLocale() === 'ar' ? 'تعديل عرض فحص طبي' : 'Edit Medical Test Offer')

@section('content')
<div class="max-w-4xl mx-auto">
    <form action="{{ route('admin.medical-test-offers.update', $medicalTestOffer) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <x-admin.ui.form-card :title="app()->getLocale() === 'ar' ? 'تفاصيل العرض' : 'Offer Details'">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    @else
                        <x-admin.ui.select name="laboratory_id" required>
                            ...
                            @foreach($laboratories as $lab)
                                <option value="{{ $lab->id }}" @selected(old('laboratory_id', $medicalTestOffer->laboratory_id) == $lab->id)>{{ $lab->name }}</option>
                            @endforeach
                        </x-admin.ui.select>
                    @endif
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <x-admin.ui.select name="medical_test_id" required>
                        ...
                        @foreach($medicalTests as $test)
                            <option value="{{ $test->id }}" @selected(old('medical_test_id', $medicalTestOffer->medical_test_id) == $test->id)">
                ...
                <div>
                    ...
                    <x-admin.ui.input type="number" step="0.01" name="price" :value="old('price', $medicalTestOffer->price)" required placeholder="0.00" />
                </div>

                <!-- Offer Price -->
                <div>
                    <x-admin.ui.label for="offer_price">{{ app()->getLocale() === 'ar' ? 'سعر العرض' : 'Offer Price' }}</x-admin.ui.label>
                    <x-admin.ui.input type="number" step="0.01" name="offer_price" :value="old('offer_price', $medicalTestOffer->offer_price)" placeholder="0.00" />
                </div>

                <!-- Discount -->
                <div class="md:col-span-2">
                    <x-admin.ui.label for="discount">{{ app()->getLocale() === 'ar' ? 'الخصم (%)' : 'Discount (%)' }}</x-admin.ui.label>
                    <x-admin.ui.input type="number" step="0.01" min="0" max="100" name="discount" :value="old('discount', $medicalTestOffer->discount)" />
                </div>
            </div>
        ...
        <div class="mt-8 flex items-center justify-end gap-3">
            ...
            <x-admin.ui.button type="submit" color="green">
                {{ app()->getLocale() === 'ar' ? 'تحديث العرض' : 'Update Offer' }}
            </x-admin.ui.button>
        </div>
    </form>
    </div>
