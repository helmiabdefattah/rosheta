...
@section('title', app()->getLocale() === 'ar' ? 'إضافة منظمة خيرية' : 'Create Charitable Organization')
@section('page-title', app()->getLocale() === 'ar' ? 'إضافة منظمة خيرية جديدة' : 'Create New Charitable Organization')
@section('page-description', app()->getLocale() === 'ar' ? 'أدخل تفاصيل المنظمة الخيرية الجديدة أدناه' : 'Enter the details of the new charitable organization below')

@section('content')
<div class="max-w-4xl mx-auto">
    <form action="{{ route('admin.charitable-organizations.store') }}" method="POST">
        @csrf
        ...
        <x-admin.ui.form-card :title="app()->getLocale() === 'ar' ? 'معلومات المنظمة الخيرية' : 'Charitable Organization Information'">
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <x-admin.ui.input name="name" :value="old('name')" required placeholder="{{ app()->getLocale() === 'ar' ? 'مثال: جمعية خيرية' : 'e.g. Charity Organization' }}" />
                </div>
                ...
                <div>
                    ...
                    <x-admin.ui.input name="address" :value="old('address')" required placeholder="{{ app()->getLocale() === 'ar' ? 'مثال: شارع الرئيسي، رقم 10' : 'e.g. Main Street, No. 10' }}" />
                </div>
                ...
                <div>
                    ...
                    <div id="phone-numbers-container" class="space-y-2">
                        <div class="flex items-center gap-2 phone-number-item">
                            <x-admin.ui.input name="phone_numbers[]" :value="old('phone_numbers.0')" placeholder="{{ app()->getLocale() === 'ar' ? 'مثال: 01234567890' : 'e.g. 01234567890' }}" />
                ...
                <div>
                    ...
                    <div id="services-container" class="space-y-2">
                        <div class="flex items-center gap-2 service-item">
                            <x-admin.ui.input name="services[]" :value="old('services.0')" placeholder="{{ app()->getLocale() === 'ar' ? 'مثال: تقديم مساعدات غذائية' : 'e.g. Food Assistance' }}" />
                ...
                <div class="flex items-center gap-3 pt-2">
                    <input type="checkbox" name="is_active" id="is_active" value="1" class="rounded border-slate-300 text-primary focus:ring-primary" {{ old('is_active', true) ? 'checked' : '' }}>
        ...
        <div class="mt-8 flex items-center justify-end gap-3">
            ...
            <x-admin.ui.button type="submit">
                {{ app()->getLocale() === 'ar' ? 'حفظ المنظمة الخيرية' : 'Save Charitable Organization' }}
            </x-admin.ui.button>
        </div>
    </form>
</div>
...
<script>
    $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.data) {
                            ...
                            // If old city was selected, trigger city change to load areas
                            if (oldCityId) {
                    ...
                    success: function(response) {
                        if (response.success && response.data) {
                            response.data.forEach(function(area) {
                                ...
                                if (oldAreaId && area.id == oldAreaId) {
                                    option.attr('selected', 'selected');
                                }
        ...
        // Trigger governorate change if old governorate_id exists
        if (oldGovernorateId) {
