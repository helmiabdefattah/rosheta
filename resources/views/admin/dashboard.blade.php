    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-md transition-shadow duration-300 relative overflow-hidden group">
            ...
            <div class="relative z-10">
                ...
                <h3 class="text-sm font-medium text-slate-500 uppercase tracking-wider">{{ app()->getLocale() === 'ar' ? 'الأطباء' : 'Doctors' }}</h3>
                <div class="flex items-end justify-between mt-1">
                    <p class="text-3xl font-black text-slate-800 tracking-tight">{{ number_format($stats['doctors']) }}</p>
                    <a href="{{ route('admin.doctors.index') }}" class="text-sm font-semibold text-teal-600 hover:text-teal-700 flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                         {{ app()->getLocale() === 'ar' ? 'عرض' : 'View' }}
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                    </a>
                </div>
        ...
        <!-- Clinics -->
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 hover:shadow-md transition-shadow duration-300 relative overflow-hidden group">
            <div class="absolute top-0 right-0 w-32 h-32 bg-sky-50 rounded-bl-full -mr-8 -mt-8 transition-transform group-hover:scale-110 duration-500"></div>
            <div class="relative z-10">
                <div class="flex items-center justify-between mb-4">
                    <div class="w-12 h-12 bg-sky-100/80 rounded-xl flex items-center justify-center text-sky-600 shadow-sm group-hover:bg-sky-600 group-hover:text-white transition-colors duration-300">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                        </svg>
                    </div>
                </div>
                <h3 class="text-sm font-medium text-slate-500 uppercase tracking-wider">{{ app()->getLocale() === 'ar' ? 'العيادات' : 'Clinics' }}</h3>
                <div class="flex items-end justify-between mt-1">
                    <p class="text-3xl font-black text-slate-800 tracking-tight">{{ number_format($stats['clinics']) }}</p>
                    <a href="{{ route('admin.clinics.index') }}" class="text-sm font-semibold text-sky-600 hover:text-sky-700 flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                         {{ app()->getLocale() === 'ar' ? 'عرض' : 'View' }}
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                    </a>
                </div>
