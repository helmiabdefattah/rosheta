...
@section('title', app()->getLocale() === 'ar' ? 'إضافة تخصص' : 'Create Specialization')
@section('page-title', app()->getLocale() === 'ar' ? 'إضافة تخصص' : 'Create Specialization')
@section('page-description', app()->getLocale() === 'ar' ? 'أدخل بيانات التخصص' : 'Enter specialization details')
        ...
        <x-admin.ui.form-card :title="app()->getLocale() === 'ar' ? 'بيانات التخصص' : 'Specialization Details'">
            <div class="space-y-6">
                <div>
                    ...
                    <x-admin.ui.input name="name" :value="old('name')" required placeholder="e.g. Cardiology" />
                    @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <x-admin.ui.label for="slug">{{ app()->getLocale() === 'ar' ? 'Slug (اختياري)' : 'Slug (optional)' }}</x-admin.ui.label>
                    <x-admin.ui.input name="slug" :value="old('slug')" placeholder="e.g. cardiology" />
                    @error('slug')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <x-admin.ui.label for="brief">{{ app()->getLocale() === 'ar' ? 'نبذة' : 'Brief' }}</x-admin.ui.label>
                    <textarea name="brief" id="brief" rows="3" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">{{ old('brief') }}</textarea>
