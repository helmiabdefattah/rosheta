...
@section('page-title', app()->getLocale() === 'ar' ? 'تعديل منطقة' : 'Edit Area')
...
<div class="bg-white rounded-lg shadow p-6 max-w-3xl">
    <form action="{{ route('admin.areas.update', $area) }}" method="POST">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name_ar" id="name_ar" value="{{ old('name_ar', $area->name_ar) }}" required
                            class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                        @error('name_ar')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-4">
                    <label for="city_id" class="block text-sm font-medium text-slate-700 mb-2">
                        {{ app()->getLocale() === 'ar' ? 'المدينة' : 'City' }} <span class="text-red-500">*</span>
                    </label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div>
                        ...
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $area->sort_order) }}" min="0"
                            class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent">
                        @error('sort_order')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex items-center mt-6">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $area->is_active) ? 'checked' : '' }}
        ...
        <div class="mt-6 flex items-center justify-end gap-4">
            <a href="{{ route('admin.areas.index') }}" class="px-4 py-2 text-slate-700 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors">
