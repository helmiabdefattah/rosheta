...
@section('title', app()->getLocale() === 'ar' ? 'البحث عن مقدمي الخدمات' : 'Search Service Providers')
@section('page-title', app()->getLocale() === 'ar' ? 'البحث عن مقدمي الخدمات' : 'Search Service Providers')
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow">
            ...
            <div class="filter-content collapsed p-6" id="filterContent">
                <form method="GET" action="{{ route('client.service-providers.index') }}" class="space-y-4" id="filterForm">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select name="provider_type" id="provider_type" class="w-full border rounded-md p-2" required>
                                ...
                                <option value="radiology_lab" @selected(request('provider_type') === 'radiology_lab')>
                                    {{ app()->getLocale() === 'ar' ? 'معمل أشعة' : 'Radiology lab' }}
                                </option>
                                <option value="test_lab" @selected(request('provider_type') === 'test_lab')>
                                    {{ app()->getLocale() === 'ar' ? 'معمل تحاليل' : 'Test lab' }}
                                </option>
                                <option value="pharmacy" @selected(request('provider_type') === 'pharmacy')>
                                    {{ app()->getLocale() === 'ar' ? 'صيدلية' : 'Pharmacy' }}
                                </option>
                                <option value="nursing" @selected(request('provider_type') === 'nursing')>
                                    {{ app()->getLocale() === 'ar' ? 'تمريض منزلي' : 'Home nursing' }}
                                </option>
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('client.service-providers.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach($results as $item)
                            <div class="border rounded-lg p-4 hover:shadow-lg transition-shadow" data-marker-id="{{ $item->id }}">
                                ...
                                <div class="mb-2">
                                    @php
                                        $spBadgeClass = match ($providerType) {
                                            'radiology_lab', 'test_lab' => 'bg-blue-100 text-blue-800',
                                            'pharmacy' => 'bg-green-100 text-green-800',
                                            'nursing' => 'bg-violet-100 text-violet-800',
                                            default => 'bg-rose-100 text-rose-800',
                                        };
                                        $spBadgeLabel = match ($providerType) {
                                    ...
                                    @endphp
                                    <span class="px-2 py-1 text-xs rounded-full {{ $spBadgeClass }}">
                                        {{ $spBadgeLabel }}
                                    </span>
                                </div>
                                ...
                                <div class="mt-4 space-y-2">
                                    ...
                                    @elseif($providerType === 'radiology_lab')
        ...
        @else
            <div class="bg-white rounded-lg shadow p-6">
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 text-lg">
                        {{ app()->getLocale() === 'ar' ? 'يرجى اختيار المحافظة ونوع مقدم الخدمة للبحث' : 'Please select governorate and service provider type to search' }}
                    </p>
                </div>
            </div>
        @endif
