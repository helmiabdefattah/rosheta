...
@section('title', app()->getLocale() === 'ar' ? 'تعديل عنوان' : 'Edit Address')

@section('page-title', app()->getLocale() === 'ar' ? 'تعديل العنوان' : 'Edit Address')
@section('page-description', app()->getLocale() === 'ar' ? 'تعديل معلومات العنوان' : 'Update address information')
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 lg:p-8">
        <form method="POST" action="{{ route('client.addresses.update', $address) }}">
            ...
            <div class="mb-6">
                ...
                <input type="hidden" id="lat" name="lat" value="{{ old('lat', $address->location['lat'] ?? '') }}">
                <input type="hidden" id="lng" name="lng" value="{{ old('lng', $address->location['lng'] ?? '') }}">
                @error('lat')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                @error('lng')
                    ...
                    success: function(response) {
                        if (response.success && response.data) {
                            response.data.forEach(function(area) {
                                const isSelected = area.id == {{ old('area_id', $address->area_id) }};
                                areaSelect.append(
                                    $('<option></option>')
                                        .attr('value', area.id)
