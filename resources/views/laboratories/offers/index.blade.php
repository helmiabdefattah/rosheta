...
@section('content')
    <div class="bg-white rounded-lg shadow-sm mb-6">
        <div class="px-4 sm:px-6 py-4 border-b border-gray-200">
            <form method="GET" action="{{ route('laboratories.offers.index') }}" class="flex flex-col md:flex-row gap-4 md:items-end stack-toolbar-mobile" id="filterForm">
                ...
                <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    ...
                    @if(request('search') || request('status'))
                        <a href="{{ route('laboratories.offers.index') }}" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors text-center w-full sm:w-auto">
                        ...
                        @foreach($offers as $offer)
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap" data-label="{{ $l ? 'السعر الإجمالي' : 'Total Price' }}">
                                    <span class="text-sm font-semibold text-primary-600">{{ number_format($offer->total_price, 2) }} {{ $l ? 'جنيه' : 'EGP' }}</span>
                                </td>
