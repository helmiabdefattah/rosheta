...
| `images/` | 26 PNGs — profile pictures, covers and 11 post designs × 2 languages |
...
`source/content.js` holds every line that appears on the images; `source/gen.js` builds the HTML
and the design system. Rendering needs Chromium and the four Google fonts (Poppins, Inter, Cairo,
...
node gen.js                       # writes one HTML file per image
